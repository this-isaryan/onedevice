									...
									<div class="filter_panel">
										...
										<div class="list-group">
											...
											<div style="max-height: 180px; overflow-y: auto; overflow-x: hidden;">
												<?php
												$query = "SELECT brand FROM brands WHERE categories_id = 1 AND status = 1 order by brand";
										...
										<div class="list-group">
											...
											$query = "SELECT DISTINCT(ramemory) FROM product WHERE status = '1' AND ramemory IS NOT NULL ORDER BY ramemory";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
										...
										<div class="list-group">
											...
											<?php
											$query = "SELECT DISTINCT(intmem) FROM product WHERE status = '1' AND intmem IS NOT NULL ORDER BY intmem";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
										...
										<div class="list-group">
											...
											<?php
											$query = "SELECT DISTINCT(camset) FROM product WHERE status = '1' AND camset IS NOT NULL ORDER BY camset";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
											...
											<?php
											$query = "SELECT DISTINCT(mcamera) FROM product WHERE status = '1' AND mcamera IS NOT NULL ORDER BY mcamera";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
											...
											<?php
											$query = "SELECT DISTINCT(fcamset) FROM product WHERE status = '1' AND fcamset IS NOT NULL ORDER BY fcamset";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
											...
											<?php
											$query = "SELECT DISTINCT(fcam) FROM product WHERE status = '1' AND fcam IS NOT NULL ORDER BY fcam";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
										...
										<div class="list-group">
											...
											<h3>Display Type</h3>
											<?php
											$query = "SELECT DISTINCT(distype) FROM product WHERE status = '1' AND distype IS NOT NULL ORDER BY distype";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
											...
											<h3>Refresh Rate</h3>
											<?php
											$query = "SELECT DISTINCT(refrate) FROM product WHERE status = '1' AND refrate IS NOT NULL ORDER BY refrate";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
										...
										<div class="list-group">
											...
											<?php
											$query = "SELECT DISTINCT(mchip) FROM product WHERE status = '1' AND mchip IS NOT NULL";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
											...
											<?php
											$query = "SELECT DISTINCT(mPCores) FROM product WHERE status = '1' AND mPCores IS NOT NULL";
											$statement = $connect->prepare($query);
											$statement->execute();
											$result = $statement->fetchAll();
											foreach ($result as $row) {
										...
										<div class="list-group">
											...
											<div class="list-group-item checkbox android-versions" style="display: none;">
												<?php
												$query = "SELECT DISTINCT(osv) FROM product WHERE status = '1' AND categories_id = 1 AND osicon = 'android' ORDER BY osv";
												$statement = $connect->prepare($query);
												$statement->execute();
												$result = $statement->fetchAll();
												foreach ($result as $row) {
											...
											<div class="list-group-item checkbox list_border_bottom">
												<label class="label-radio"><input type="checkbox" class="common_selector input-radio radio ios" value="ios">
													<p class="label-r">iOS</p>
												</label>
											</div>
											<div class="list-group-item checkbox iOS-versions" style="display: none;">
												<?php
												$query = "SELECT DISTINCT(osv) FROM product WHERE status = '1' AND categories_id = 1 AND osicon = 'apple' ORDER BY osv";
												$statement = $connect->prepare($query);
												$statement->execute();
												$result = $statement->fetchAll();
												foreach ($result as $row) {
												?>
													<div class="list-group-item checkbox list_border">
														<label class="Container"><input type="radio" name="osv_ios" class="common_selector osv" value="<?php echo $row['osv'] ?>"><?php echo $row['osv'] ?><span class="check"></span></label>
													</div>
												<?php } ?>
											</div>
										</div>
	...
	$(document).ready(function() {
		...
		$('.android').change(function() {
			if (this.checked) {
				...
				$('.android-versions .osv').attr('disabled', false);
			} else {
				$('.android-versions').hide();
				$('.android-versions .osv').prop('checked', false);
				$('.android-versions .osv').attr('disabled', true);
				$('.ios').attr('disabled', false);
			}
		});

		$('.android-versions .osv').change(function() {
			if (this.checked) {
				$('.ios').attr('disabled', true);
				$('.iOS-versions .osv').attr('disabled', true);
			} else {
				$('.ios').attr('disabled', false);
				$('.iOS-versions .osv').attr('disabled', false);
			}
		});
		...
		$('.ios').change(function() {
			if (this.checked) {
				...
				$('.iOS-versions').hide();
				$('.iOS-versions .osv').prop('checked', false);
				$('.iOS-versions .osv').attr('disabled', true);
				$('.android').attr('disabled', false);
			}
		});

		$('.iOS-versions .osv').change(function() {
			if (this.checked) {
